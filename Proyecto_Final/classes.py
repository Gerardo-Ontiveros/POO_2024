...
class Proveedores:
    def __init__(self, nombre_compania,rfc, nombre,apellido, correo, telefono, direccion, ciudad, estado,cp,pais, id=None):
        self.id = id
    ...
    def crear(self):
        ...
        query = """
            INSERT INTO supplier (name, lastname, company, rfc, phone, email, address, city, state, postalcode, country)
            VALUES (%s, %s, %s, %s, %s,%s,%s,%s,%s,%s,%s)
        """
        values = (self.nombre, self.apellido, self.nombre_compania, self.rfc, self.telefono, self.correo, self.direccion, self.ciudad, self.estado, self.cp, self.pais)
    ...
    def actualizar(self, id_proveedor):
        ...
        try:
            query = """
                UPDATE supplier
                SET name=%s,lastname=%s,company=%s,rfc=%s,phone=%s,email=%s,address=%s,city=%s,state=%s,postalcode=%s,country=%s 
                WHERE idSupplier = %s
            """
            values = (self.nombre,self.apellido,self.nombre_compania,self.rfc,self.telefono,self.correo,self.direccion,self.ciudad,self.estado,self.cp,self.pais,id_proveedor)
            cursor.execute(query, values)
            conexion.commit()
    ...
    def eliminar(id_proveedor):
        ...
        cursor = conexion.cursor()
        try:
            query = "DELETE FROM supplier WHERE idSupplier = %s"
